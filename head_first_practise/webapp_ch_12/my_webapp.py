...
@app.route('/entry/')	#	Page used to enter phrase and letters for search
def entry_page() -> 'html':
	return render_template('entry.html',
			the_title='Welcome to search4letters online!')

@app.route('/viewlog/')	#	View log of searches that have been done
@check_logged_in
def view_log()->'html':
...
@app.route('/login')	#	Login to view log
def do_login()-> str:
	session['logged_in'] = True
	return redirect('/')

@app.route('/logout')	#	Logout of page
def do_logout() -> str:
